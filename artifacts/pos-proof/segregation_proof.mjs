...
const created = await post("/org/nodes", { parent_id: parent.id, role: "STOREFRONT", name: "Proof Till SF" }, admin);
...
// 5) Browser: bind the new till, PIN in, ring a sale, see receipt
...
for (const d of cashier.pin.split("")) await pg.locator(`button:text-is("${d}")`).first().click();
