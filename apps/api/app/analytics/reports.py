...
from app.analytics.timezones import PLATFORM_DEFAULT_TZ, to_local
from app.db.base import utcnow
from app.models.orders import OrderItem
from app.models.payments import Payment, Transaction
from app.models.tenancy import Outlet

# Reports are presented in a report timezone (default SG). Timestamps are stored naive-UTC; `to_local`
# (DST-correct via zoneinfo) converts at read for day/hour bucketing. The route resolves `tz` + the
# date-range bounds (`timezones.local_day_bounds_utc`). See app/analytics/timezones.py.
...
def _txns(db: Session, merchant_id: str | None, allowed_outlets: set[str] | None,
    ...
    # The range is HALF-OPEN [start, end): `end` is the start of the day after the last local day, so
    # there's no microsecond gap and no double-count at midnight.
...
def sales_timeseries(db: Session, *, merchant_id: str | None, allowed_outlets: set[str] | None,
                     ...
                     tz: str = PLATFORM_DEFAULT_TZ) -> list[dict]:
    now = utcnow()
    if start is None:
    ...
    for t in txns:
        key = _period_key(to_local(t.created_at, tz), granularity)   # bucket by report-local day/hour
...
def peak_hours(db: Session, *, merchant_id: str | None, allowed_outlets: set[str] | None,
    ...
    for t in txns:
        h = to_local(t.created_at, tz).hour   # report-local hour (peak hours in trading time, not UTC)
        buckets[h]["orders"] += 1
...
def forecast(db: Session, *, merchant_id: str | None, allowed_outlets: set[str] | None,
             horizon_days: int = 7, window: int = 7, tz: str = PLATFORM_DEFAULT_TZ) -> dict:
    ...
    """
    today = to_local(utcnow(), tz).date()   # "today" in the report tz (daily keys are report-local)
    daily = sales_timeseries(db, merchant_id=merchant_id, allowed_outlets=allowed_outlets,
                             granularity="day", days=max(window * 3, 30), tz=tz)
