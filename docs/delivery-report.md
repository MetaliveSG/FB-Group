...
## 2. What was built
- **Backend** (`apps/api`): FastAPI + SQLAlchemy 2.0, **45 tables**, **140 API endpoints**, 32 Alembic migrations.
...
.venv/bin/python -m app.seed && .venv/bin/python -m pytest      # 312 backend tests
...
## 6. Test results
- **Backend: 312 passed** (pytest, 52 files) — `artifacts/pytest_results.txt`. Also covers the **KDS / fulfilment**
...
- **Frontend: 66 passed** (Vitest) — `artifacts/frontend_test_results.txt`.
...
## 8. API documentation
Swagger `/docs`, ReDoc `/redoc`, machine spec `artifacts/openapi.json` (140 endpoints),
human reference `docs/api.md`.

## 9. Database schema (45 tables)
...
Postgres stack. **The app runs; 312 backend + 66 frontend tests pass; the QR→order→
...
for a PoC (explicitly listed in §10). Verified 2026-06-11 (counts + state current as of R42 —
binary parent-gated module flags + Wallet 4th toggle + /platform tree-grid; KDS kitchen display;
two-axis service options w/ SEA-first default; customer ready-to-collect notifications; 1-week token).
