...
## 2. What was built
- **Backend** (`apps/api`): FastAPI + SQLAlchemy 2.0, **41 tables**, **116 API endpoints**, 16 Alembic migrations.
...
## 8. API documentation
Swagger `/docs`, ReDoc `/redoc`, machine spec `artifacts/openapi.json` (116 endpoints),
...
Postgres stack. **The app runs; 216 backend + 45 frontend tests pass; the QR→order→
