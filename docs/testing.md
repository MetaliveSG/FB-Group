...
| `test_breadtalk_member_tree.py` | org-tree | **unlimited member-tree + node-RBAC proof** (BreadTalk Group, Enterprise spanning 2 Merchants, depth 0–4): spine holds arbitrary depth (`sellable_under`/`outlet_ids_under` path-prefix); a role at any node cascades DOWN its subtree (CEO@Enterprise→both merchants all outlets; Brand Manager→its brand's outlet only; Stall Operator→parent outlet); sibling + upline + cross-merchant isolation; every tier account logs in |
