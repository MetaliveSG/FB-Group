...
```
Migration chain: **8 revisions** (initial schema → rewards catalog/wheel/tasks/owner →
redemption voucher_code → opportunities/activities → pipeline_type + merchant settings →
jackpot_prizes → customers.gender → menu_items.image_url), single head, verified to upgrade (40 tables) and downgrade cleanly.
On container start,
